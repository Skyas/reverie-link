...
**本次 Phase 3 仅实现前两项**，后两项推迟：
1. 三阶混合记忆架构（优先）
2. 游戏态势感知系统（次之）
3. RVC v2 便携式绿色环境 → 推迟至依赖冻结后
4. 最终打包优化 → 推迟至 Phase 4

## [Phase 3] 游戏态势感知开发策略

**决策：先通用，后专用。**
- 第一步：实现无接口、无日志的通用视觉流方案（策略C：暴力全局/局部视觉流），覆盖所有游戏。
- 第二步：在通用方案跑通后，再针对具体游戏（CS2 GSI / LOL Live Client API）做专项优化。
理由：通用方案是底层基础，专项优化是上层增强，顺序不可倒置。

## [Phase 3] 三阶记忆架构——摘要模型

**当前决策：摘要任务（核心档案更新、历史压缩）直接复用用户已配置的主对话模型，不单独配置。**
理由：摘要调用频率低、token 消耗小，单独配置增加用户心智负担，性价比低。

**Phase 4 待实现（可配置方案）**：
在「全局设置」Tab 的记忆配置区增加一个下拉选择器：
> **【摘要模型】**  `[与主对话模型相同 ▼]`
> 展开选项：与主对话模型相同 / 自定义模型名称

选择"自定义"时，仅出现一个"模型名"输入框；API Key 与 Base URL 完全复用主模型配置（同厂商换便宜小模型，无需跨厂商）。
设计原则：最小化配置，只增加一个控件，不重复厂商/Key 配置。

## [Phase 3] 三阶记忆架构——向量数据库选型

**决策：ChromaDB。**
理由：自带持久化、内置 embedding 与语义检索、安装简单（`pip install chromadb`），适合本项目轻量单机场景。FAISS 需手动管理序列化，更适合大规模生产环境，不选。
...
### 4-7. 摘要模型可配置
**背景**：Phase 3 摘要任务暂时复用主对话模型。用户若希望用更便宜的轻量模型单独承担摘要工作，
当前版本无法满足。
**方案**：在「全局设置」Tab 记忆配置区增加下拉选择器：
- 选项一：与主对话模型相同（默认）
- 选项二：自定义模型名称（仅增加一个模型名输入框，API Key 与 Base URL 复用主模型配置）

设计原则：最小化控件，不重复厂商/Key 配置项。