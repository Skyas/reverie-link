...
.field-label    { font-size: 12px; font-weight: 600; color: #4A4A6A; display: flex; align-items: center; gap: 6px; }
.field-note     { font-size: 11px; font-weight: 400; color: #9B8FB0; }
.required       { color: #FFAABB; }
.field-hint     { font-size: 11px; color: #9B8FB0; padding-left: 2px; }
.field-input {
    width: 100%; padding: 9px 12px;
    border: 1.5px solid rgba(212,184,224,0.45); border-radius: 10px;
    background: #FFFFFF; font-size: 13px; color: #4A4A6A;
    font-family: inherit; outline: none;
    transition: border-color 0.2s, box-shadow 0.2s;
}
.field-input:focus      { border-color: #7EC8E3; box-shadow: 0 0 0 3px #C5E8F4; }
.field-input::placeholder { color: #9B8FB0; opacity: 0.7; }
.field-input:disabled   { background: rgba(200,190,220,0.12); cursor: not-allowed; }
.field-readonly         { background: rgba(200,190,220,0.12) !important; color: #9B8FB0; cursor: not-allowed; }
.field-select {
    width: 100%; padding: 9px 12px;
    border: 1.5px solid rgba(212,184,224,0.45); border-radius: 10px;
    background: #FFFFFF; font-size: 13px; color: #4A4A6A;
...
.field-select:focus { border-color: #7EC8E3; }
.field-range        { width: 100%; accent-color: #7EC8E3; }
.field-row          { display: flex; gap: 12px; }
.toggle-row         { display: flex; align-items: center; justify-content: space-between; }

.action-row { display: flex; align-items: center; justify-content: flex-end; gap: 12px; padding-top: 4px; }
.save-btn {
    ...
    cursor: pointer; box-shadow: 0 3px 12px rgba(180,140,200,0.12);
    transition: opacity 0.2s, transform 0.15s;
}
.save-btn:hover  { opacity: 0.88; }
.save-btn:active { transform: scale(0.97); }

.divider { height: 1px; background: rgba(212,184,224,0.45); margin: 2px 0; }

.global-section          { display: flex; flex-direction: column; gap: 10px; }
.global-section-disabled { opacity: 0.45; pointer-events: none; }
.global-section-title    { font-size: 13px; font-weight: 700; color: #4A4A6A; display: flex; align-items: center; gap: 8px; }
.coming-badge            { font-size: 10px; font-weight: 600; color: #9B8FB0; background: #FFE4EC; padding: 2px 8px; border-radius: 10px; }

/* 弹框 */
.dialog-overlay {
    position: fixed; inset: 0;
    background: rgba(100,80,120,0.25); backdrop-filter: blur(4px);
    display: flex; align-items: center; justify-content: center; z-index: 200;
}
.dialog-box {
    width: 300px; background: #FFFFFF; border-radius: 18px;
    padding: 22px 20px 18px; box-shadow: 0 8px 32px rgba(126,87,194,0.18);
    display: flex; flex-direction: column; gap: 14px;
}
.dialog-title   { font-size: 15px; font-weight: 700; color: #4A4A6A; }
...
.dialog-confirm {
    ...
    background: linear-gradient(135deg, #A8D8EA, #FFB7C5);
    color: white; font-size: 13px; font-weight: 600; font-family: inherit; cursor: pointer;
}
...
.header::before {
    ...
    width: 100px; height: 100px; border-radius: 50%; background: rgba(255,255,255,0.15);
}
.header::after {
    content: ""; position: absolute; bottom: -30px; left: 40%;
    width: 120px; height: 120px; border-radius: 50%; background: rgba(255,255,255,0.1);
}
.header-icon  { font-size: 11px; color: rgba(255,255,255,0.7); letter-spacing: 2px; }
.header-title { font-size: 18px; font-weight: 700; color: white; text-shadow: 0 1px 4px rgba(0,0,0,0.1); }
.header-sub   { font-size: 12px; color: rgba(255,255,255,0.8); }

.tabs {
    display: flex; padding: 12px 20px 0;
    background: var(--c-surface); border-bottom: 1.5px solid var(--c-border); flex-shrink: 0;
}
.tab-btn {
    flex: 1; padding: 9px 8px;
    border: none; border-radius: 10px 10px 0 0;
    background: transparent; cursor: pointer;
    font-size: 12px; font-family: inherit; color: var(--c-text-soft);
    font-weight: 500; transition: all 0.2s;
    display: flex; align-items: center; justify-content: center; gap: 4px;
}
.tab-btn:hover { color: var(--c-text); background: var(--c-pink-light); }
.tab-btn.active {
...
.content {
    ...
    padding: 16px 20px 20px; background: var(--c-bg);
}

/* tab-content 由各子组件自己渲染，父组件只提供外层容器 */
:deep(.tab-content) { display: flex; flex-direction: column; gap: 14px; }
</style>