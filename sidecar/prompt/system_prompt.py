...
_HARD_CONSTRAINT = (
    ...
    "【表达要求】即使面对相似的场景或话题，也必须使用不同的句式、语气和词汇，绝不能像机器一样重复完全相同的原话。"
    "【事件记录】如果当前发生了特定的互动（如用户与你的特殊互动，或是画面发生了值得记住的变化）,"
    "请在回复末尾添加 <event>简要动作描述</event>（如 <event>被亲了1次</event>）。\n"
    "★注意：普通的日常闲聊、简单的屏幕切换、或者你自己的情绪变化【绝对不要】记录为 event！仅记录值得被记录的事件。"
)
